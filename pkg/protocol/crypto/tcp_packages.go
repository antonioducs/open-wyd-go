package crypto

import (
	"encoding/binary"
	"errors"
)

var keys = []byte{
	,
}

var ErrInvalidChecksum = errors.New("invalid packet checksum")

func Decrypt(input []byte) ([]byte, error) {
	if len(input) < 4 {
		return nil, nil
	}

	buffer := make([]byte, len(input))
	copy(buffer, input)

	length := binary.LittleEndian.Uint16(buffer[0:2])

	if int(length) > len(buffer) {
		length = uint16(len(buffer))
	}

	keyIncrement := int(keys[int(buffer[2])*2])

	var (
		keyResult   byte
		checksumEnc int
		checksumDec int
	)

	for i := 4; i < int(length); i++ {
		checksumEnc += int(buffer[i])

		keyIndex := (keyIncrement&0x800000ff)*2 + 1

		if keyIndex < len(keys) {
			keyResult = keys[keyIndex]
		} else {
			keyResult = 0
		}

		iterator := i & 3

		switch iterator {
		case 0:
			buffer[i] -= byte(int(keyResult) << 1)
		case 1:
			buffer[i] += byte(int(keyResult) >> 3)
		case 2:
			buffer[i] -= byte(int(keyResult) << 2)
		case 3:
			buffer[i] += byte(int(keyResult) >> 5)
		}

		checksumDec += int(buffer[i])
		keyIncrement++
	}

	calculatedChecksum := byte(checksumEnc - checksumDec)

	if buffer[3] != calculatedChecksum {
		return nil, ErrInvalidChecksum
	}

	return buffer, nil
}

func Encrypt(buffer []byte) []byte {
	if len(buffer) < 4 {
		return nil
	}

	length := binary.LittleEndian.Uint16(buffer[0:2])

	if int(length) > len(buffer) {
		length = uint16(len(buffer))
	}

	hashKey := buffer[2]
	keyIncrement := keys[hashKey*2]

	var (
		keyResult   byte
		checksumEnc byte
		checksumDec byte
	)

	for i := 4; i < int(length); i++ {
		checksumDec += buffer[i]

		keyResult = keys[int(keyIncrement)*2+1]
		iterator := i & 3

		switch iterator {
		case 0:
			buffer[i] += keyResult << 1
		case 1:
			buffer[i] -= keyResult >> 3
		case 2:
			buffer[i] += keyResult << 2
		case 3:
			buffer[i] -= keyResult >> 5
		}

		checksumEnc += buffer[i]
		keyIncrement++
	}

	buffer[3] = checksumEnc - checksumDec

	return buffer
}
